...
exports.anime = async function(wikiid){
  let promise = new Promise((resolve, reject) => {
    const url = 'https://ru.wikipedia.org/w/api.php?action=parse&format=json&pageid=' + wikiid;
...
exports.msg = function(data, name, url){
  ...
  let msg = '';
  if(name){
    msg += 'Новая серия **' + name + '**!';
  }else{
    msg += 'Новая серия аниме/манги/сериала!'
  }
  msg += '\n';
  if(typeof e != 'undefined'){
    msg += 'И это у нас уже **' + e.serie + '** серия!';
    msg += '\n';
    // msg += 'Название серии: **' + e.name + '**';
    // msg += '\n';
  }
  msg += 'Название сезона: **' + data[1] + '**';
  msg += '\n';
  if(typeof e != 'undefined'){
    msg += e.simpletext;
  }
  // msg += 'Также должны выйти ёще **' + String(data[0].length - data[2]) + '** серий.';
  if(url){
    msg += '\n';
    msg += 'Подробнее: ' + url;
  }
